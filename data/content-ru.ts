...
const contentRu: CVData = {
  ...
  projects: {
    ...
    items: [
      {
        ...
        lead: 'Модуль сканирования документов в стиле Chrome с масштабированием, вращением, подгонкой по высоте/ширине и удалением страниц. Синхронизированная прокрутка и выделение между боковой панелью и основным просмотрщиком через Intersection Observer. Связка Chrome-расширения и десктопного приложения давала веб-сессии доступ к сканеру: браузер не может работать с оборудованием напрямую, поэтому команды шли через расширение в десктоп-приложение, а отсканированные страницы возвращались в сессию в реальном времени.',
        did: [
    ...
    sideHeading: 'Пет-проекты, в свободное время.',
    sideItems: [
      {
        title: 'Storyblok to Payload Migration',
        when: '2026',
        role: 'Личный проект',
        lead: 'Монорепозиторий на Turborepo, демонстрирующий полную миграцию headless-CMS со Storyblok на Payload CMS v3. Node-скрипт сопоставляет контент-модели и записи Storyblok с типизированными коллекциями Payload, а фронтенд на Next.js 16 сохраняет Visual Editor Storyblok и ISR по вебхукам. Архитектурные решения зафиксированы в ADR.',
        did: [
          'Скрипт миграции с маппингом моделей Storyblok на типизированные коллекции Payload — Lexical rich text, медиа, связи.',
          'Структура монорепозитория на Turborepo (web / cms / migration) с npm workspaces.',
          'Фронтенд на Next.js 16 (App Router) с интеграцией Visual Editor Storyblok и ревалидацией ISR по вебхукам.',
          'Ключевые компромиссы задокументированы в виде ADR.',
        ],
      ...
      },
      {
        ...
        when: '2026',
        role: 'Личный проект',
        lead: 'End-to-end приложение на Next.js 15 + Payload CMS v3, исследующее продакшен-паттерны headless-коммерции: типизированные коллекции с контролем доступа, Lexical-редактор с кастомными блоками, оплата через Stripe и полный набор автотестов — всё в Docker для локальной разработки.',
        did: [
          'Кастомные коллекции Payload с типизированными правилами доступа и Lexical-редактор с кастомными блоками.',
          'Оплата через Stripe с хранением в Vercel Blob и транзакционными письмами через Resend.',
          'Полное покрытие: интеграционные тесты Vitest + E2E на Playwright.',
          'Локальное окружение в Docker через docker-compose.',
        ],
  ...
  ui: {
    ...
    footer: {
      footerCredit: 'Сделано с душой · Обновлено июнь 2026',
    },
  },
};
